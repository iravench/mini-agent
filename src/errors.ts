...
const ERROR_HINTS: Record<string, (ctx: AfterToolCallContext) => string> = {
  ...
  read_file: (ctx) => {
    const args = ctx.args as { path?: string };
    return `\n\nHint: Check that the path exists and is a file (not a directory). Use bash with ls or find to locate the file.`;
  },
  write_file: (ctx) => {
    const args = ctx.args as { path?: string };
...
const RETRYABLE_PATTERNS =
  /overloaded|rate.?limit|too many requests|429|500|502|503|504|service.?unavailable|server.?error|network.?error|connection.?error|timeout|timed? out/i;
